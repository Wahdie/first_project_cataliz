...
<Properties>
  ...
  <Property name="block_at" type="timestamp">
  ...
  <Property name="dissapering_message" type="boolean">
    Fitur untuk memunculkan pesan dalam jangka waktu sementara.
  </Property>
  <Property name="unread_count" type="number">
    Fitur untuk menampilkan seluruh pesan yang belum terbaca.
  </Property>
</Properties>
...
<Row>
  ...
  <Col sticky>
    ...
    const totalUnreadCount = conversationData.data.reduce((sum, conversation) => sum + conversation.unread_count, 0);

    // Update the JSON with total unread count for all conversations
    conversationData.total_unread_count = totalUnreadCount;

    // Update unread_count for a specific conversation (e.g., first conversation)
    const specificConversationIndex = 0;
    conversationData.data[specificConversationIndex].unread_count = 5;
