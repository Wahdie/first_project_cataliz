...
<Properties>
  ...
  <Property name="group" type="object">
    The group object for the group who sent the message.
  </Property>
  <Property name="message_type" type="string">
    Jika berupa text biasa, meesage_type menggunakan "text", jika voice chat menggunaakan "voice_chat".
  </Property>
  <Property name="message" type="object">
    The message content. Pesan dapat berupa voice chat atau text biasa. Jika berupa text biasa, meesage_type menggunakan "text", jika voice chat menggunaakan "voice_chat".
  </Property>
  ...
  <Col sticky>
    ...
    ```json {{ title: 'Response' }}
    {
      ...
      "data": [
        ...
        "message": {
          "text": "You’re what the French call ‘les incompetents.’",
          "voice_url": "https://example.com/voice/gWqY86BMFRiH5o11.wav",
          "voice_duration": "00:01:30"
        },
        "reactions": [],
        "read_at": null,
        "created_at": 692233200,
...
<Row>
  ...
  <Col sticky>
    ...
    ```json {{ title: 'Response' }}
    {
      ...
      "groups": { },
      "message_type" : "voice_chat", // atau "text"
      "message": {
          "text": "null",
          "voice_url": "https://example.com/voice/gWqY86BMFRiH5o11.wav",
          "voice_duration": "00:01:30"
...
<Row>
  <Col>
    ...
    This endpoint allows you to delete messages from your conversations. Note: This will permanently delete the message or you can select 'delete for me' to delete the message form your conversation only.
...
// Menghapus pesan berdasarkan ID pesan untuk pengguna sendiri (delete for me)
async function deleteMessageForMe(messageId) {
  try {
    const response = await client.messages.delete(messageId);
    console.log('Message deleted for me:', response);
  } catch (error) {
    console.error('Error deleting message:', error);
  }
}

// Menghapus pesan berdasarkan ID pesan untuk semua orang (delete for everyone)
    async function deleteMessageForEveryone(messageId) {
      try {
        // Anda perlu mengirim permintaan khusus untuk delete for everyone
        const response = await client.messages.deleteForEveryone(messageId);
        console.log('Message deleted for everyone:', response);
      } catch (error) {
        console.error('Error deleting message for everyone:', error);
      }
    }

    const messageIdToDelete = 'SIuAFUNKdSYHZF2w';

    // Menghapus pesan untuk pengguna sendiri
    deleteMessageForMe(messageIdToDelete);

    // Menghapus pesan untuk semua orang
    deleteMessageForEveryone(messageIdToDelete);
